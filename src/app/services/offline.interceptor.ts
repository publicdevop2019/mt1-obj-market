        ...
        if (environment.mode === 'offline') {
            if (['delete', 'put', 'post', 'patch'].includes(req.method.toLowerCase())) {
                ...
                if (req.url.includes('orders/')) {
                    ...
                    return of(new HttpResponse({ status: 200, headers: var0 })).pipe(delay(this.DEFAULT_DELAY))
                }
            ...
            if (['get'].includes(req.method.toLowerCase())) {
                ...
                if (req.url.includes('orders/id')) {
                    let header = new HttpHeaders();
                    let var0=header.set('Location', '100')
                    return of(new HttpResponse({ status: 200, headers: var0 })).pipe(delay(this.DEFAULT_DELAY))
                }
                if (req.url.includes('/confirm')) {
                    return of(new HttpResponse({ status: 200, body: {paymentStatus:true} })).pipe(delay(this.DEFAULT_DELAY))
                }
                if (req.url.includes('orders/')) {
